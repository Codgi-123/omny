...
enum SharedInbox {
    ...
    private static var queueURL: URL? {
        FileManager.default
            ...
            .appendingPathComponent("share-queue.json")
    }

    static func append(text: String, urlString: String?) {
        guard let url = queueURL else { return }
        var items = load(from: url)
        items.append(SharedItem(text: text, urlString: urlString, savedAt: .now))
