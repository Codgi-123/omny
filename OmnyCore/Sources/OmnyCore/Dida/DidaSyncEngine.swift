...
public protocol TodoSyncStore: Sendable {
    ...
    /// 远端任务落地：按 didaTaskID upsert
    func upsertFromRemote(_ task: DidaTask) async throws
    /// 远端未完成列表里消失的已同步任务 → 视为在滴答侧被完成
...
/// 已知取舍：滴答侧"删除"任务在本地会表现为"已完成"而非删除
/// （未完成列表里消失无法区分删除和完成），自用可接受。
    ...
    func pull() async throws {
        ...
        for todo in locals {
            guard let didaID = todo.didaTaskID,
                  !todo.needsPush, !todo.isDeletedLocally, !todo.isCompleted,
                  !remoteOpenIDs.contains(didaID)
            else { continue }
            try await store.markCompletedFromRemote(didaTaskID: didaID)
        }
    }
}
