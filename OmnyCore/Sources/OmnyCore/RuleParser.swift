...
public struct RuleParser: Parser {
    ...
    /// 记账交易动词：出现其一才可能是动账短信（配合金额特征双命中，降低误判）
    static let expenseVerbs = ["消费", "支出", "支付", "付款", "收入", "入账", "到账", "转账", "交易", "扣款", "扣费", "还款"]
    ...
    /// 收入措辞：命中其一判为收入，否则默认支出（银行短信支出居多）
    static let incomeKeywords = ["收入", "入账", "到账", "退款", "退回", "转入", "工资", "报销"]
