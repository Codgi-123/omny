...
2. **CI macOS 编译通过**（GitHub Actions app-macos job，commit 962627b 绿）。注：`app-macos` 现已改为仅手动触发（见 `CLAUDE.md`「分支与 CI」），复现编译需在 Actions 页 Run workflow。
...
- Windows 侧编译不了 App 层，只能靠 CI（macOS job，现为手动触发，见 `CLAUDE.md`「分支与 CI」）+ 真机。
